...
환경이 확인되지 않았다면 먼저 `environment-setup.md`에서 Python과 `openpyxl`을 검사한다. 아래 `<python>`은 macOS에서
`python3`, Windows PowerShell에서 `py -3`이다. Windows에서 변경할 때는 대상 Excel 파일을 Excel 앱에서 닫아야 원자
교체가 가능하다.
...
- 저장된 파일이 이동·삭제됐으면 새 파일을 만들지 말고 새 절대경로를 받아 `profile reconnect`한다.
...
```bash
<python> <tool> search --workbook <절대경로>/매물장.xlsx \
...
<python> <tool> add --workbook <절대경로>/매물장.xlsx --record-json '<JSON>' --expected-sha <sha>
<python> <tool> update --workbook <절대경로>/매물장.xlsx --id P001 --changes-json '<JSON>' --expected-sha <sha>
<python> <tool> complete --workbook <절대경로>/매물장.xlsx --id P001 --expected-sha <sha>
<python> <tool> detail-upsert --workbook <절대경로>/매물장.xlsx --id P001 --changes-json '<JSON>' --expected-sha <sha>
```
...
데이터가 많아도 원본 전체를 대화에 싣지 않고 결정적 도구의 상위 결과와 집계만 사용한다. 공유·동시 편집이 잦으면
Google Sheets가 더 적합할 수 있다.
