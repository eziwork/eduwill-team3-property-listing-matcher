...
def require_openpyxl() -> None:
    if load_workbook is None or Workbook is None:
        ...
        fail(
            "로컬 Excel 기능에는 openpyxl이 필요합니다",
            hint=f"{launcher} -m pip install --user openpyxl",
        )
...
def fsync_directory(path: Path) -> None:
    if os.name == "nt":
        return  # Windows does not expose portable directory fsync semantics.
...
def publish_new_file(temporary: Path, destination: Path) -> None:
    """Publish a verified new file without overwriting an existing destination."""
    try:
        os.link(temporary, destination)
        return
    except FileExistsError:
        fail("기존 파일을 덮어쓰지 않습니다", path=str(destination))
    except OSError:
        pass  # Some Windows, network, and removable filesystems do not support hard links.
...
def file_lock(target: Path) -> Iterable[None]:
    ...
    with lock_path.open("a+b") as lock_handle:
        if os.name == "nt":
            if msvcrt is None:
                fail("Windows 파일 잠금 기능을 사용할 수 없습니다")
            ...
            if fcntl is None:
                fail("POSIX 파일 잠금 기능을 사용할 수 없습니다")
...
def command_doctor() -> dict[str, Any]:
    ...
    openpyxl_installed = load_workbook is not None and Workbook is not None
    try:
        import openpyxl

        openpyxl_version: str | None = openpyxl.__version__
    except ImportError:
    ...
    return {
        ...
        "openpyxl": {
            ...
            "install_command": None if openpyxl_installed else f"{launcher} -m pip install --user openpyxl",
        },
        "local_excel_ready": python_supported and openpyxl_installed,
        "google_sheets_check": "Google Drive 도구와 로그인 계정은 Google 연결 게이트에서 별도로 확인합니다",
    }
...
def command_profile(args: argparse.Namespace, store_path: Path) -> dict[str, Any]:
    if args.profile_action in {"list", "show"}:
        ...
        with file_lock(store_path):
            ...
            if args.profile_action == "reconnect":
                current = data["profiles"].get(args.name)
                if not current:
                    fail("프로필을 찾을 수 없습니다", name=args.name)
                if current.get("access") not in {"local-xlsx", "google-sheet"}:
                    fail(
                        "지원하지 않는 저장 방식의 프로필입니다",
                        name=args.name,
                        access=current.get("access"),
                        hint="새 프로필로 Google Sheets 또는 로컬 Excel을 연결하세요",
                    )

                profile = dict(current)
                if current["access"] == "local-xlsx":
                    if not args.workbook:
                        fail(
                            "로컬 Excel 프로필은 --workbook으로만 다시 연결할 수 있습니다",
                            name=args.name,
                            hint="Google Sheets로 바꾸려면 새 프로필을 연결하세요",
                        )
                    workbook_path = resolve_path(args.workbook)
                    validate_workbook_pair(workbook_path, current["listing_sheet"], current["detail_sheet"])
                    profile["workbook_path"] = str(workbook_path)
                else:
                    if not args.sheet_id:
                        fail(
                            "Google Sheets 프로필은 --sheet-id로만 다시 연결할 수 있습니다",
                            name=args.name,
                            hint="로컬 Excel로 바꾸려면 새 프로필을 연결하세요",
                        )
                    sheet_id = extract_sheet_id(args.sheet_id)
                    profile.update(
                        sheet_id=sheet_id,
                        spreadsheet_url=f"https://docs.google.com/spreadsheets/d/{sheet_id}/edit",
                        connector=args.connector or current["connector"],
                        account=args.account or current["account"],
                    )
            ...
            if args.profile_action == "disconnect":
                current = data["profiles"].get(args.name)
                if not current:
                    fail("프로필을 찾을 수 없습니다", name=args.name)
                is_active = data.get("active_profile") == args.name
                if is_active:
                    if bool(args.activate) == bool(args.allow_no_active):
                        fail(
                            "활성 프로필을 해제하려면 --activate <다른이름> 또는 --allow-no-active 중 하나가 필요합니다",
                            name=args.name,
                        )
                    if args.activate:
                        if args.activate == args.name:
                            fail("해제할 프로필을 다시 활성화할 수 없습니다", name=args.name)
                        replacement = data["profiles"].get(args.activate)
                        if not replacement:
                            fail("대체 활성 프로필을 찾을 수 없습니다", name=args.activate)
                        profile_view(replacement, verify_local=True)
                        data["active_profile"] = args.activate
                    else:
                        data["active_profile"] = None
                elif args.activate or args.allow_no_active:
                    fail(
                        "비활성 프로필 해제에는 활성 프로필 옵션이 필요하지 않습니다",
                        name=args.name,
                        active_profile=data.get("active_profile"),
                    )

                disconnected = profile_view(current)
                del data["profiles"][args.name]
                write_profile_store(store_path, data)
                return {
                    "ok": True,
                    "profile_store": str(store_path),
                    "disconnected_name": args.name,
                    "disconnected_profile": disconnected,
                    "active_profile": data["active_profile"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    profile_reconnect = profile_actions.add_parser("reconnect")
    profile_reconnect.add_argument("--name", required=True)
    reconnect_source = profile_reconnect.add_mutually_exclusive_group(required=True)
    reconnect_source.add_argument("--workbook")
    reconnect_source.add_argument("--sheet-id")
    profile_reconnect.add_argument("--connector", default=None)
    profile_reconnect.add_argument("--account", default=None)
    profile_reconnect.add_argument("--activate", action="store_true")
    profile_disconnect = profile_actions.add_parser("disconnect")
    profile_disconnect.add_argument("--name", required=True)
    profile_disconnect.add_argument("--activate", metavar="OTHER_NAME")
    profile_disconnect.add_argument("--allow-no-active", action="store_true")
    ...
    subparsers.add_parser("doctor", help="Check Python and local Excel prerequisites")
